...
namespace MailSubscriptionFunctionApp.Functions.Subscriptions
{
    ...
    /// </summary>  
    [OpenApiOperation(operationId: "CreateMailSubscription", tags: new[] { "Subscriptions" },
    ...
    public class CreateMailSubscription : BaseFunction
    {
        ...
        public CreateMailSubscription(IGraphSubscriptionClient graphClient, IMailSubscriptionRepository repository, ILogger<CreateMailSubscription> logger, ICustomTelemetry telemetry)
        ...
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Function, "post", Route = "subscriptions")] HttpRequestData req)
        {
            ...
            try
            {
                ...
                if (!body.RootElement.TryGetProperty("userId", out var userIdElement) || string.IsNullOrWhiteSpace(userIdElement.GetString()))
                {
                ...
                await response.WriteAsJsonAsync(new { message = "Subscription created successfully.", subscriptionId = subscription.SubscriptionId });
